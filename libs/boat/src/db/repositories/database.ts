...
import { GenericFunction } from '@libs/boat/constants';
  ...
  implements RepositoryContract<T>
{
   ...
   */
  async chunk(where: T, size: number, cb: GenericFunction): Promise<void> {
    const query = this.query();
    query.where(where);
  ...
  async updateAndReturn(
    ...
    const query = this.query();
    query.where(where).patch(setValues).returning('*');
    returnOne && query.first();
   ...
   * @param inputs
   */
  async bulkInsert(inputs: T[]): Promise<T[]> {
    return this.query().insert(inputs).returning('*') as unknown as T[];
  }
}
